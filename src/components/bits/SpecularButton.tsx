...
vec3 col=uBaseColor*base+uLineColor*hi;fragColor=vec4(col,clamp(base+hi,0.0,1.0));}`;

export function SpecularButton({
  children = 'Explore', size = 'lg', radius = 2, tint = '#ffffff', tintOpacity = 0,
  blur = 0, textColor = '#f5f5f5', lineColor = '#ffffff', baseColor = '#6b6258',
  ...
  useEffect(() => {
    ...
    let proximityAmount = propsRef.current.autoAnimate ? 1 : 0;
    const onPointerMove = (event: PointerEvent) => {
      ...
      pointerAngle = distance === 0
        ? Math.atan2(2 / rect.height, -2 / rect.width) + ((event.clientX - centerX) / Math.max(rect.width, 1)) * .45
    ...
    let angle = 2.4;
    let idleAngle = 2.4;
    let brightness = propsRef.current.autoAnimate ? 1 : 0;
    ...
    const update = (now: number) => {
      ...
      const steer = props.followMouse && pointerAngle !== null && (!props.autoAnimate || proximityAmount > 0);
      const target = reduced ? 2.4 : steer ? pointerAngle as number : idleAngle;
      const difference = ((target - angle + Math.PI * 3) % (Math.PI * 2)) - Math.PI;
      angle += difference * (1 - Math.exp(-delta * 7));
      const brightnessTarget = reduced || props.autoAnimate ? 1 : proximityAmount;
  ...
  return <button ref={buttonRef} type={type} disabled={disabled} onClick={onClick} className={`specular-button specular-button--${size} ${className}`.trim()} style={{ '--sb-radius': `${radius}px`, '--sb-tint': tint, '--sb-tint-opacity': tintOpacity, '--sb-blur': `${blur}px`, '--sb-text-color': textColor } as React.CSSProperties}>
