...
export function AtlasSpotlight() {
  ...
  const [radius, setRadius] = useState(() => window.innerWidth < 640 ? 170 : window.innerWidth < 1024 ? 240 : 320);

  useLayoutEffect(() => {
    const updateMedia = () => {
      setRadius(window.innerWidth < 640 ? 170 : window.innerWidth < 1024 ? 240 : 320);
    };
  ...
  return (
    ...
    >
      <SpotlightReveal imageSrc={CAMPUS_IMAGE} baseRadius={radius} />
      <div className="spotlight-instruction spotlight-reveal-item"><span>01</span> Move to reveal the archive</div>
